...
2. **No `CallHandle`/`CallStatus` normalised return type yet** — `start_call`/`get_call`
   ...
   a second adapter's real payload exists to normalise *against* would mean designing
...
`no_answer`/`busy`/`voicemail` aren't in CALL-E's *error* taxonomy — they show up as
...
This is the *same* discipline this session already applied one layer up, for the same
...
a single *real* vendor's data would be guessing, not abstracting" (§1's own words,
...
half is "and don't design the shape *before* the first one either," which is the
...
and "raw audio bytes." When Plivo's handler is built (P5b), *then* look at what the
...
abstraction has to be reworked *and* both adapters retrofitted to it.
...
| Scalability | N/A at this layer |
| Team familiarity | Reuses the *reasoning* from P1's own precedent, not the pattern itself yet |
...
- **Important: the message vocabulary is asymmetric, unlike Twilio's shared
  ...
  *different* vocabulary: `playAudio`, `checkpoint`, `clearAudio`. A `PlivoProvider`
...
accepts a credential as *either* an `api-subscription-key` header *or* a standard
...
and per P1's own gap (`ISSUES.md`, this session's review), the *stub* for tests should
...
- **"Sarvam is the recommended/pre-selected default" in the Agents tab (§4, product):**
  ...
  signup) before §4 ships a default; until then, the Agents tab can list Sarvam as *an*
...
*what this campaign sounds like*, same altitude as `goal_template`), resolved at
