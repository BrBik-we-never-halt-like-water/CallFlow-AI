...
| [#77](#77--channel_members_insert-s-rls-check-verified-the-inserter-never-the-person-being-added)                                  | S1  | `channel_members_insert`'s RLS check verified the inserter, never the person being added                             | database       | it-24 | **FIXED**        |
| [#78](#78--invitations_repoaccept-could-abort-its-own-transaction-on-a-wrong-email-or-racing-accept)                              | S2  | `invitations_repo.accept()` could abort its own transaction on a wrong-email or racing accept                        | backend        | it-24 | **FIXED**        |
| [#79](#79--messages_repo-send_message-never-inserted-org_id-so-every-real-send-500d)                                              | S1  | `messages_repo.send_message()` never inserted `org_id`, so every real send 500'd                                     | backend        | it-24 | **FIXED**        |
| [#80](#80--test_anonymous_sees_nothing-assumed-anon-s-access-is-always-denied-via-rls-never-via-a-missing-grant)                  | S4  | `test_anonymous_sees_nothing` assumed `anon`'s access is always denied via RLS, never via a missing grant             | backend        | it-24 | **FIXED**        |
| [#81](#81--channel_members-had-a-delete-policy-but-no-delete-grant)                                                                | S2  | `channel_members` had a `DELETE` policy but no `DELETE` grant                                                          | database       | it-25 | **FIXED**        |
| [#82](#82--an-org-admin-renaming-a-channel-they-hadnt-joined-got-back-nothing-even-though-the-rename-worked)                       | S2  | An org admin renaming a channel they hadn't joined got back nothing, even though the rename worked                     | database       | it-25 | **FIXED**        |
| [#83](#83--useorgrealtime-broke-entirely-the-moment-a-second-component-watched-the-same-table)                                    | S1  | `useOrgRealtime` broke entirely the moment a second component watched the same table                                   | web            | it-25 | **FIXED**        |
| [#84](#84--patch-messagesid-edit-500d-on-every-real-edit)                                                                          | S1  | `PATCH .../messages/{id}` (edit) 500'd on every real edit                                                              | backend        | it-25 | **FIXED**        |
...
## Iteration 24 - 2026-08-15 · internal team chat (RUNBOOK_JATIN_PART_3.md)

### #77 - `channel_members_insert`'s RLS check verified the inserter, never the person being added
...
### #78 - `invitations_repo.accept()` could abort its own transaction on a wrong-email or racing accept

**S2 · FIXED · backend · `apps/api/app/database/repositories/invitations.py`**

Found while fixing `#77` - the exact same shape of bug, in code this phase didn't touch
...
### #79 - `messages_repo.send_message()` never inserted `org_id`, so every real send 500'd
...
### #80 - `test_anonymous_sees_nothing` assumed `anon`'s access is always denied via RLS, never via a missing grant
...
## Iteration 25 - 2026-08-15 · Teams-parity chat: search, group management, read state, message edit/delete, pagination
...
pattern iteration 24's #79 already established, repeating here because the fix for it
(build a local dev stack, drive the feature over real HTTP and a real browser) is exactly
what caught these too.

### #81 - `channel_members` had a `DELETE` **policy** but no `DELETE` **grant**
...
### #82 - An org admin renaming a channel they hadn't joined got back nothing, even though the rename worked
...
### #83 - `useOrgRealtime` broke entirely the moment a second component watched the same table
...
### #84 - `PATCH .../messages/{id}` (edit) 500'd on every real edit

**S1 · FIXED · backend · `apps/api/app/api/v1/routes/messages.py`**

Same shape as `#79`: `messages_repo.edit_message()`'s `UPDATE ... RETURNING` has no join to
...
## Iteration 26 - 2026-08-16 · deep technical audit of chat: correctness, scalability, concurrency

A system-design-level audit of chat only (frontend, API, repositories, schema, RLS,
Realtime, tests), not a re-run of Iteration 25's feature checklist. Three real, verified
...
### #85 - Starting a DM with the same teammate twice created two separate conversations
...
### #86 - The chat unread-badge hook re-ran chat's heaviest query on every message sent anywhere in the organisation, for every open tab
...
### #87 - Cursor pagination could silently skip or repeat a message under an exact-timestamp tie
...
## Iteration 27 - 2026-08-17 · chat RLS: org departure did not revoke Realtime/PostgREST access

Follow-up to Iteration 26's #85 discussion of organisation isolation. That round's own re-evaluation
...
### #88 - Chat RLS treated `channel_members`/`created_by` as permanent, never re-checking current organisation membership
...
**Depends on / Blocks:** Iteration 26 #85.

## Iteration 28 - 2026-08-18 · chat RLS: `channel_members_select` was the one policy #88's fix didn't cover

Direct follow-up to #88. That fix's own migration deliberately left `channel_members_select`
...
### #89 - `channel_members_select`'s member branch had no live organisation-membership check

**S2 · FIXED · database · `apps/api/alembic/versions/202608100000_team_chat_channels_and_messages.py (now folded into the single consolidated chat migration)`**

`is_channel_member(channel_id) or has_org_role(...)` - the first branch, like every other one #88
...
`channel_members` row deliberately left in place to isolate the policy from the cleanup #88 also
...
severity than #88, but the same root cause and the same reachable path.
...
**Verified**, following the same before/after discipline as #88:
...
**Depends on / Blocks:** Iteration 27 #88.
