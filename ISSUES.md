...
**Root cause.** `org_repo.create()` inserted into `organisations` *with `RETURNING`*
...
While reproducing #28, the API's logs showed *two* chained tracebacks: the real
...
on *every* error from *any* route using `as_user()`, not just this one.
...
each disposition mean visually") maps *both* `escalated` and `unreachable` to the
...
reporting the *original* organisation as `active`, every time, including across a
full page reload.
...
Settings tabs; both are about *which workspace* you're in, not the product's own
...
**Related, not duplicate:** #31 (last iteration) was the org *switcher* not taking
...
started attaching the pinned org id, a *stale* pin became newly capable of producing a
...
**Impact.** Someone who left the *name* blank saw a red-bordered phone field and "Add a
...
`setPaused(true)` (the *exact same* state change as "Pause run") and then fired a toast
...
Neither endpoint checked *which* role was being granted: `set_member_role` accepted any
...
organisation's *last* owner, not demoting or removing a non-last one, `#13`/`protect_last_owner`),
...
CLAUDE.md §4b's stated model of an API check *and* an RLS check, neither alone:
...
- **API.** `app/auth/permissions.py` gains `can_grant_role(caller_role, target_role)`:
  ...
  every other role may only grant a role *strictly* below its own — Admin gets
...
- **Critical — the invitee could still self-escalate a pending invite's role.**
  ...
  update *any column* of their own pending invitation, including `role`, checking only
...
- **`memberships_insert`'s invitation branch didn't pin `user_id`.**
  `has_valid_invitation(org_id, role)` only checked that *the caller* held a matching
...
- **`can_grant_role` alone never checked the target member's *current* role.** In an org
  with two Owners, an Admin could still demote or remove one of them —
  `memberships_update`/`memberships_delete` looked only at the *new* value being written,
...
choice, and it is *broader* than the two-Owner scenario above — an Admin can now only
update or remove a member whose current role is operator or viewer, which also means an
Admin can no longer act on *another Admin's* row, not only an Owner's. Disclosed here
rather than left implicit. No residual gap is known open in this policy family as of this
fix; the one thing found but *not* fixed here, because it's an unrelated pre-existing
...
`HTTPException(403)` for every denial (granted-role *and* target-role), plus
...
scoped to the invitee's own email — so when the *inviter* (an Owner/Admin, a different
...
plain UPDATE policy does nothing to satisfy the *column-level* `GRANT` that finding 1
...
needs UPDATE privilege on every column in that `do update set` list for the *entire
statement* to plan — checked once at executor start, regardless of whether a conflict
...
advertised action (same severity class as `#28`), introduced by a fix for a *different*
...
**Verification.** Reproduced the regression live *before* fixing it (a fresh, non-
...
the count and list not updating is visible *immediately*, without ever leaving the
...
**Pre-existing, not introduced by round 3.** Traced back: the *old* `--text-mute`
(`#6b787e`) against the *old* `--surface` (`#f4f6f5`, both from before this round's other
...
an `EmptyState` *inside* it, the wrapper itself always renders. So a brand-new
...
sans-serif fallback with *neither* font loaded — worse than the current Inter Tight
mismatch, not better.
...
*never* valid for the caller making it, with no indication in the picker itself, is
...
was checked for the same class of bug and found *not* buggy: `CampaignRunner` always
