...
export function DocsShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto grid max-w-(--container-marketing) gap-8 px-4 pt-10 sm:px-6 lg:grid-cols-[200px_minmax(0,1fr)] xl:grid-cols-[200px_minmax(0,1fr)_180px]">
      {/* ---- Section nav ------------------------------------------------- */}
                  ...
                  return (
                    <li key={docPage.slug}>
                      <Link
                        ...
                        className={cn(
                          "-mx-2 block rounded-sm px-2 py-1.5 text-small transition-colors duration-(--dur-micro)",
                          active
        ...
        {(prev || next) && (
          ...
          >
            {prev ? (
              <Link
                href={`/docs/${prev.slug}`}
                className="flex flex-1 flex-col gap-1 rounded-md border border-rule p-3 transition-colors hover:bg-surface-hover"
              >
                <Eyebrow>Previous</Eyebrow>
                <span className="text-small font-medium text-text">{prev.title}</span>
              </Link>
            ) : null}
            {next ? (
              <Link
                href={`/docs/${next.slug}`}
                className="flex flex-1 flex-col gap-1 rounded-md border border-rule p-3 text-right transition-colors hover:bg-surface-hover"
              >
                <Eyebrow>Next</Eyebrow>
                <span className="text-small font-medium text-text">{next.title}</span>
              </Link>
            ) : null}
          </nav>
        )}
        ...
        {headings.map((heading) => (
          <li key={heading.id}>
            <a
              href={`#${heading.id}`}
              className={cn(
                "-ml-px block border-l py-1 text-small transition-colors duration-(--dur-micro)",
