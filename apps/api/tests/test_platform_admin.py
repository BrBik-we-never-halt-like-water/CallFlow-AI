...
async def test_the_elevated_read_returns_another_tenants_runs(
    ...
    campaign_id = f"plat-camp-{uuid.uuid4().hex[:8]}"
    await db.execute(
        """
        insert into public.campaigns (id, org_id, name, goal_template, created_by)
        values ($1, $2, 'Support probe', 'Ask one question.', $3)
        """,
        campaign_id,
        tenant.org_id,
        tenant.user_id,
    ...
    await db.execute(
        """
        insert into public.runs (id, org_id, campaign_id, status, total, started_by)
