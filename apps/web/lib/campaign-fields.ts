...
export const EDITOR_FIELD_TYPES: {
...
}[] = [
  {
    value: 'string',
    label: 'Text',
    hint: "Anything in the contact's own words",
  },
  ...
  { value: 'boolean', label: 'Yes / no', hint: 'A single true-or-false fact' },
  {
    value: 'date',
    label: 'Date',
    hint: 'A specific day, returned as an ISO date',
  },
  {
    value: 'enum',
    label: 'Choice',
    hint: 'One of a fixed set of options you list',
  },
];
...
export function renderGoalPreview(
  ...
  return template
    ...
    .replace(
      /\{context\[(\w+)\]\}/g,
      (_, key: string) => contact.context[key] ?? '',
    )
    .replace(
      /\{context\.(\w+)\}/g,
      (_, key: string) => contact.context[key] ?? '',
