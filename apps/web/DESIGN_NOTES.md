...
**Superseded (`ISSUES.md` #180). Both halves were right; the coverage changed.**
At 19-of-57 monochrome was correct - a mix of full-colour logos and grey letters
reads as broken rather than varied, so everything went grey.
`scripts/fetch-brand-logos.mjs` now fetches each provider's mark from its own
site into `public/brands/` and gets **56 of 57**, which inverts the argument: the
monogram is one genuine exception (PlayAI, whose domains no longer resolve)
rather than two thirds of the page, and the coherent grid is the one made of real
logos. `brand-paths.ts` is deleted; `lib/brand-logos.ts`, generated by that
script, replaces it.

This does not breach §9, which is about *this product* reading as first-party and
already sanctions vendor names in `provider_credentials` "because the operator
picked it" - exactly what an integrations directory is. The monochrome decision
was about visual coherence, not de-branding.
...
- **Self-hosted, not proxied.** A favicon-service request per card would tell a
  ...
  blank offline.
- **The logo fills the plate.** About half of these are app-icon style with the
  vendor's colour bled to the edges and half are transparent marks, so padding
  would frame the first kind as a small square inside a square.
  `overflow-hidden` plus `object-contain` handles both without `BrandMark`
  knowing which it has, and an `onError` falls back to the monogram so a bad file
  looks intentional rather than like a render bug.
- **A theme-reactive SVG loses to its raster.** An SVG carrying its own
  `prefers-color-scheme` block resolves it against the *viewer's OS*, not the
  plate it sits on, so it vanishes on half the theme combinations. Monochrome
  alone is not the trigger - ElevenLabs and Perplexity are monochrome vectors and
  both read correctly.

`components/app/agentic/provider-icons.tsx` shares the artwork now, which ends a
real inconsistency: it had its own resolver, so Twilio was a red logo on the
Agents tab and the letter "T" on Integrations. Its `simple-icons` tier still wins
where it has an entry - a vector taking `currentColor` is the better asset.
