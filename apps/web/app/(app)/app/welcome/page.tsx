 ...
 *
 * It ends with the user having watched a real run produce a typed result, because
...
export default function WelcomePage() {
  ...
  async function startFirstRun() {
    ...
    setStarting(true);
    try {
      const { run_id } = await api.startRun(campaignId, toContactInputs(rows));
      setRunId(run_id);
      setStep(3);
      refresh();
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      {step === 2 ? (
        <Panel className="flex flex-col gap-5 p-5 sm:p-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button loading={starting} disabled={!campaignId} onClick={startFirstRun}>
              Start run
            </Button>
            <span className="self-center text-small text-text-mute">
              This places a real call to each contact above.
            </span>
          </div>
        </Panel>
      ) : null}
      ...
      {/* ---- 4 · The result --------------------------------------------- */}
      {step === 3 ? (
        <Panel className="flex flex-col gap-5 border-l-2 border-l-lamp-brass p-5 sm:p-6">
          <div className="flex flex-col gap-2">
          ...
          {firstResult ? (
            ...
            <p className="text-small text-text-mute">
              Results appear here as each call settles.
            </p>
          )}
