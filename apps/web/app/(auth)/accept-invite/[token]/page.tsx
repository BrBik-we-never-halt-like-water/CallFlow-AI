...
export default function AcceptInvitePage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        if (!cancelled) setPreview({ valid: false, reason: "not_found", org_name: null, role: null, email: null });
      })
      .finally(() => {
  ...
  async function acceptAndEnter() {
    ...
    try {
      ...
      setFormError(error instanceof Error ? error.message : "That didn't work.");
      setSubmitting(false);
    }
  }
    ...
    return (
      ...
      >
        <p className="text-small text-text-dim">
          {(preview?.reason && REASON_COPY[preview.reason]) ?? "This invitation isn't valid."}
        </p>
      </AuthCard>
    );
  }
      ...
      <div className="flex flex-col gap-4">
        <Panel sunken className="flex flex-col gap-2 p-3">
          <div className="flex items-baseline justify-between gap-3">
            ...
            <span className="text-small font-medium text-text">{preview.org_name}</span>
          </div>
          <div className="flex items-baseline justify-between gap-3">
        ...
        {alreadySignedIn ? (
          <>
            {formError ? <p className="text-small text-lamp-flare-text">{formError}</p> : null}
            <Button size="lg" className="w-full" loading={submitting} onClick={acceptAndEnter}>
              Join {preview.org_name}
            </Button>
          </>
        ) : (
          <form onSubmit={submitNewAccount} noValidate className="flex flex-col gap-4">
