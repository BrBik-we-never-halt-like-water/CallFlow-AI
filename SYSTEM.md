...
| Per-teammate usage-credit cap | unset (no per-teammate ceiling; the organisation-wide balance is the only gate) | `member_credit_allocations.monthly_credit_cap_paise`, editable admin/owner via `PATCH .../members/{id}/credit-cap`. Money in paise, checked against this calendar month's spend at the run gate alongside the org-wide balance (`ISSUES.md` #144). Superseded, kept as a marker: a per-teammate *daily call* allowance (`daily_allocation`) used to sit alongside this and was retired (`ISSUES.md` #146) |
