...
| `database/repositories/runs.py`                 | Run + call-outcome persistence, raw asyncpg. `lookup_owner_for_webhook()` is the one exception - runs on an anonymous connection, via a SECURITY DEFINER function, for the webhook receiver                                                                                                                                                          | `create_run()`, `append_outcome()`, `finish_run()`, `get_run()`, `list_outcomes()`, `list_runs()`, `lookup_owner_for_webhook()` |
