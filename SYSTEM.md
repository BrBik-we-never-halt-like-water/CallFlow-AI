...
- **`(app)/app`** - dashboard (`/app`), `agentic` + `new` + `[id]` (the Agents tab - the voice-agent builder), `runs` + `new` + `[id]`, `escalations`, `contacts`, `chat` (internal team chat - channels/DMs, §3/§5; `chat-shell.tsx` is the component the route renders, parameterised by `channelId`), `integrations`, `profile`, `organisation` + `new`, `settings` + 3 panes (`safety`, `api-keys`, `billing`). **`campaigns` + `new` + `[id]` are deleted** - campaigns went with ADR-8 and an agent is what a run binds to now. `/app/settings/integrations` survives only as a redirect to the top-level `/app/integrations`, which moved into the primary nav because connecting a carrier and a speech vendor is what a new organisation has to do before anything works at all. Organisation and Team are their own route, `/app/organisation` (`Suspense`-wrapped for `useSearchParams()`, `?tab=team`/`?tab=sharing` select the Team/Sharing panes) - not a dialog and not a Settings pane; `/app/settings` and `/app/settings/team` both redirect to `/app/settings/safety` so pre-existing generic "Settings" links still resolve; "Organisation" and "Settings" live in the account menu (`user-menu.tsx`'s dropdown) and in the sidebar's footer section, and are the only two `NAV_ITEMS` entries `PRIMARY_NAV_ITEMS` filters out - they are lower-frequency than the rest, while Chat, Agents and Integrations are working features someone checks often. The footer is a second path to the same three destinations (Profile/Organisation/Settings), not a replacement for the account menu, since the sidebar disappears below `lg` and the account menu is mobile's only way to reach them (or to sign out). Creating a second organisation is a dedicated two-step page, `/app/organisation/new` (name, then an optional logo - logo upload has to be a second step because Storage RLS scopes the upload path by `org_id`, which doesn't exist until the create call returns). There is no `/app/welcome` - the mandatory org-setup step + its skippable profile follow-up are **not routes at all**; `OnboardingGate` renders them as a modal over whatever page is active (see §3/§12), specifically to avoid the two-independent-`useSession()`-instances bug a page-per-step version had (`ISSUES.md`)
