...
| Verified    | 30 API endpoints (all but 3 authenticated) · 50 built routes · **189 backend tests** (31 cross-tenant/cross-role RLS) · eslint + `tsc` clean · `alembic check` no drift   |
...
│   │   ├── alembic/                env.py + 25 revisions
│   │   ├── tests/                  18 files, 267 tests
│   │   └── pyproject.toml
│   └── web/                        Next.js
...
| **Runs**                          | Start (always live, idempotent via `Idempotency-Key`, optional per-run ceiling/allowlist that can only tighten the organisation's own), watch live, cancel (stops the next contact, not one already in conversation), open a call's transcript, export CSV                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `POST /api/v1/runs`, `GET /api/v1/runs`, `GET /api/v1/runs/{id}`, `POST /api/v1/runs/{id}/cancel`                                                                  | ✅ org-scoped Postgres, updated as each call resolves; a process restart fails any run left `running`/`canceling`                                                                                                                               |
...
| `api/v1/routes/runs.py`                         | `/api/v1/runs` - start/list/get, org-scoped, background execution. `GET /{run_id}/calls/{provider_call_id}/events` proxies CALL-E's developer event log on demand (`EngineGateway.list_events()`) - richer than status polling, but not fetched automatically for every call | `router`                                                                                                      |
...
build base outcome (masked phone)
  ...
  → _poll_until_done()     [asyncio.to_thread per poll]   2s interval, reports each status change
                                           → _await_settled_duration() once status goes terminal:
                                             the engine's top-level `status` and its nested
                                             attempt's `completed_at` are not written
                                             atomically, so a terminal response can still have
                                             a null `completed_at` - up to 3 re-fetches at 1.5s
                                             before giving up (`ISSUES.md` #63)
  → _extract_result() / _extract_transcript() / _extract_duration()
  → triage() → return
```
...
`GET /`, `GET /api/health`, `GET /api/v1/invitations/{token}`, and
`POST /api/v1/webhooks/calle/{secret}` (secret-in-path, not a bearer token - see its own
entry below) requires a bearer token
...
### `POST /api/v1/webhooks/calle/{secret}`

CALL-E's terminal-event webhook - the fast path for run outcomes; polling
(`CampaignRunner._poll_until_done`) stays the backstop for anything this misses. No bearer
token - `secret` must match `CALLFLOW_WEBHOOK_SECRET` (constant-time compare), since
CALL-E's webhooks are unsigned. A wrong or absent secret → `404`, not `401`/`403`, so a
prober learns nothing about whether the path means anything. Past the secret check, every
failure mode (unknown `run_id`, a payload missing what a `CampaignRunner`-placed call
always has) acknowledges with `200 {"ok": true}` rather than raising - there's nothing a
non-2xx would fix that logging and moving on doesn't already cover, and it would only make
CALL-E retry a delivery this deployment can never resolve regardless. Resolves identity via
`database.anonymous()` + the SECURITY DEFINER `lookup_run_owner_for_webhook()` (migration
`202608091200_calle_webhook_run_lookup.py`) - `privileged.acquire()` must never appear in a
request handler (CLAUDE.md), and this is the same `anonymous()` + narrow-lookup-function
shape `GET /api/v1/invitations/{token}` already established. The actual write then goes
through the ordinary `database.as_user()` path, as the run's own starter.
_Used by:_ CALL-E, once `CALLFLOW_PUBLIC_API_URL` and `CALLFLOW_WEBHOOK_SECRET` are both set.
...
`max_calls_per_run`/`allowlist` are optional, per-run overrides of this organisation's own
`org_safety_settings` (`ISSUES.md` it-15) - **tighten-only**, applied by the pure
`apply_run_override()` (`app/domain/safety.py`). A requested ceiling above the organisation's own
is silently capped, never honoured past it; a requested allowlist intersects with a non-empty
organisation allowlist (narrows only) and stands alone only when the organisation has none set.
`Permission.RUNS_START` (operator+) can therefore never use this to reach a guard value
`Permission.SAFETY_WRITE` (admin/owner) hasn't already permitted. `calls_per_window`/
`window_minutes`/`daily_budget` are not per-run overridable - those stay whole-organisation
resources. The web composer clamps to the organisation's own ceiling client-side before
submitting, so the API's cap is a correctness backstop for a race, not the normal path.
...
**Idempotent via an optional `Idempotency-Key` header** (`ISSUES.md` #55). A replay of the
same key for the same organisation returns the original `run_id`/`total` untouched - no
new dial, no rate-limit charge, no second row. Backed by a partial unique index on
`public.runs (org_id, idempotency_key) where idempotency_key is not null`; a race between
two identically-keyed concurrent requests resolves via `insert ... on conflict ... do
nothing returning id` - the loser releases the rate-limit slots it had reserved and
returns the winner's run. Nothing requires the header; a caller that omits it is
unaffected. `apps/web`'s run composer generates one UUID per submit attempt and keeps it
across a failed retry.

**Suppression, resolved once per run - and before the rate-limit check** (`ISSUES.md`
#58). Before creating the run, the handler hashes every contact's phone with
`phone_hash()` and checks each against the org's real `public.suppressions` table
(`suppressions_repo.is_suppressed`). The set of matching hashes is handed to
`CampaignRunner` as `suppressed_hashes`, and `check_dial_allowed` skips any contact whose
hash is in that set - one query for the whole run rather than one per contact.
`limiter.check()` is charged for `len(contacts) - len(suppressed)`, not the raw contact
count, so a suppressed contact - who was never going to be dialled - doesn't reserve a
slot from the daily budget or rate window.
...
ends. `finish_run` marks the run `completed`, `failed`, or `canceled` when the loop exits.

### `POST /api/v1/runs/{run_id}/cancel`

Requires `Permission.RUNS_START` - reused rather than a new narrower permission, since
whoever may spend the organisation's money starting a run may stop one early
(`ISSUES.md` #57). Sets `status = 'canceling'` and `cancel_requested_at = now()`
immediately (idempotent - a second call just re-confirms the same state). → `404` unknown
run · `400` if the run is already `completed`/`failed`/`canceled` ("...there's nothing
left to cancel").

`CampaignRunner.run()` checks `cancel_requested_at` **between** contacts only, never
mid-call - there is no way to interrupt a call already in conversation (the voice engine
has no cancel operation). So the real guarantee is "no further contacts are dialled," not
"stops instantly": a call already ringing when cancel is requested finishes naturally. The
run then lands on a new terminal `canceled` status. Contacts never reached get no outcome
row at all - `total` vs. however many outcomes exist already shows how far it got.
...
the same as `stats.completed` below. `status` is one of `running` / `canceling` /
`canceled` / `completed` / `failed`. `started_by*` is joined to `public.users`, `null` if
...
`stats` is built by a pure `_compute_stats()` helper (`app/api/v1/routes/runs.py`,
`tests/test_run_stats.py`) - `ISSUES.md` #8, fixed in it-14. Every rate is now over
**resolved** outcomes only (`disposition != "in_flight"`): `completed`, `escalated`,
`auto_closed`, and `needs_human_pct` all share that same denominator, and `in_flight` is
reported as its own field rather than folded into either side of a percentage. Previously
`auto_closed`/`needs_human_pct` were computed over *all* outcomes including in-flight
ones, so the percentage read artificially low while a run was still going - fixed.

`safety_snapshot` (`ISSUES.md` #60) is a **permanent record** of the exact guards this run
was governed by - the same `EffectiveSafety` object `start_run()` resolved for its own dial
gate and rate-limit check, stored once at creation, never a live read of this
organisation's *current* `org_safety_settings`. `null` only for a run created before this
field existed (`public.runs.max_calls_per_run` etc., migration `a3f7c9e2b6d8`) - never
backfilled, since defaulting a legacy row to today's settings would fabricate a history it
never actually had. `guardsFromSnapshot()` (`components/app/safety-bar.tsx`) can still
render it through the same `SafetyBar` component a live guard bar uses, but nothing on
`/app/runs/[id]` calls it anymore - the "Guards for this run" panel was removed from that
page by explicit request (it-17); the field is still written and still returned by the API,
just not surfaced in this view. Reintroducing that display, or removing the snapshot
columns themselves, are two different changes - only the former has happened.
...
No cursor pagination · no RFC 9457 problem details (errors are FastAPI's `{"detail":
"..."}`) · no webhooks in or out · **no dry-run or simulate mode of any kind** - every run
dials for real from the first request an org ever makes · **no way to interrupt a call
already in conversation** - cancelling a run only stops the *next* contact from being
dialled (see above; the voice engine itself has no cancel operation). It now **does**
have an `/api/v1` prefix throughout, Bearer-token auth, per-org scoping (RLS-enforced) on
everything except the three endpoints listed at the top of this section, an optional
`Idempotency-Key` on run creation, and a real cancel endpoint (both `ISSUES.md` it-14).
...
| `duration_seconds`   | `float \| None`  | `None`      | `(completed_at - started_at)` on the same "final" attempt the transcript is read from - **not** a `duration_seconds` field from the engine, which does not exist in CALL-E's response at all (`ISSUES.md` #61, same bug class as #52's transcript key). `None` can also mean the terminal response just hadn't caught up yet when polled - see `_await_settled_duration()` above and `ISSUES.md` #63 |
...
Run `status`: `running`, `canceling`, `canceled`, `completed`, `failed` (`canceling`/
`canceled` added `ISSUES.md` it-14 - see §5's cancel endpoint)
...
| `select.tsx` `switch.tsx` `checkbox.tsx` | Radix Select / Switch / Checkbox + `RadioGroup`. `Select`'s value is passed explicitly as `Select.Value`'s children rather than left to Radix's automatic mirroring - left implicit, Radix portals the selected `Select.Item`'s own (unstyled) `ItemText` span into the trigger, which has no `nowrap` of its own and wraps a long label instead of the `truncate`/`min-w-0` on the outer node taking effect. `overflow-hidden` on the trigger is the backstop (`ISSUES.md` #62) |
| `dialog.tsx`                             | `Dialog` (centered, sizes `sm`/`md`/`lg`/`xl`, `contentClassName` to opt out of the default padding) and `Sheet` (right-hand slide-in, still used by `contact-grid.tsx`), shared overlay, focus trap. Transcript viewers (`/app/runs/[id]`, `/app/escalations`) use `Dialog size="xl"` |
...
| `/app/escalations`                                 | Filter reason/campaign, sort oldest/newest, open transcript in a centered dialog, call back / reassign / mark resolved. Renders 20 at a time behind an `IntersectionObserver` sentinel (client-side only - the full list is already in memory, there is no server-side pagination API for this view) | derived; no endpoint                                                                            |
...
| `CALLFLOW_PUBLIC_API_URL`                    | `""`    | This API's own publicly-reachable base URL - needed to hand CALL-E a `webhook_url`. Empty ⇒ polling only, no webhook_url sent                                                                                                                 |
| `CALLFLOW_WEBHOOK_SECRET`                    | `""`    | Shared secret in `/api/v1/webhooks/calle/{secret}` - CALL-E's webhooks are unsigned, so this is the entire trust boundary. Empty ⇒ the receiver refuses every request                                                                          |
...
**Tests** - 267 collected across 17 files (verified via `pytest --collect-only -q`).
...
| `test_email.py`                | 5     | `EmailGateway`'s Resend request shape, the domain-verification rejection message, generic 403/422 staying generic, network failures - `ISSUES.md` #51                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `test_run_stats.py`            | 4     | `_compute_stats()` - pure, plain dicts, no database - `ISSUES.md` #8                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `test_crypto.py`               | 4     | Round-trips, nonce uniqueness, missing-key and tampered-ciphertext both fail closed. Pure - no database                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `test_engine.py`               | 5     | `EngineGateway`'s conformance to the `VoiceProvider` protocol, against a second stub implementation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| F18     | Runs and orchestration                                  | ⚠️ Partial | **Runs and outcomes are now Postgres-backed**, updated as each call resolves. A real cancel endpoint now exists (`running`/`canceling`/`canceled`/`completed`/`failed`) and a process restart now fails any orphaned `running`/`canceling` row instead of leaving it stuck forever (`ISSUES.md` #56, #57). `POST /api/v1/runs` also now accepts an optional `Idempotency-Key` (`ISSUES.md` #55). Still sequential and single-process within a run, and cancelling can't interrupt a call already in conversation - only the next one; no calling-window enforcement                                                                                                                                                                                                                                                                                                                                                            |
