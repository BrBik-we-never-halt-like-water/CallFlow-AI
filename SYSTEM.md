...
drift between request and decision - ISSUES.md #77); if it moved, the request is
...
- **campaign**: clones it - a new id, `created_by = requester`, independent from that
  ...
  a plain insert - see `ISSUES.md` #76 for why a plain insert fails under RLS for any
...
`ISSUES.md` iteration 25: `POST /api/v1/runs` resolves the caller's own ceiling once per
...
| Per-teammate daily credits  | unset (no per-teammate ceiling; org-wide budget is the only gate) | `member_credit_allocations.daily_allocation`, editable admin/owner via `PATCH .../members/{id}/credits`. **1 credit = 1 connected call** - a dial that never rings through doesn't spend one (`ISSUES.md` iteration 25) |
...
| `/app/escalations`                                 | Filter reason/campaign, sort oldest/newest, open transcript, call back myself, reassign (admin/owner), mark resolved - live-synced across teammates. Operators/admins/owners with `sharing:request` additionally see a "Team escalations" directory with a "Request to help" action per open row not already theirs | `GET /api/v1/escalations`, `POST .../assign`, `POST .../resolve`, `GET .../directory`, `POST /api/v1/share-requests` |
