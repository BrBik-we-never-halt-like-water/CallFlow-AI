...
> actually authenticate — `current_user()` now accepts a Supabase session *or* a
...
- a **CallFlow API key** (`cfk_…`, programmatic access — Settings → API keys), detected by
  ...
  The key's *role* is re-read from `memberships` fresh on every call rather than cached on
...
```json
{
  ...
  "extra_fields": [
    { "key": "party_size", "type": "integer", "description": "Number of travellers." }
  ],
  "region": "IN",
  "language": "en",
...
```json
{
  ...
  "previews": [
    ...
    { "name": "Bad Row", "error": "phone must be E.164 (e.g. +15555550100), got '98765'" }
  ]
}
```
...
```json
{
  "campaign_id": "travel-discovery",
  "contacts": [
    { "name": "Aditi", "phone": "+15555550100", "region": null, "language": null,
...
```json
{
  ...
  "finished_at": "2026-08-05T09:12:05.108Z",
  "error": null,
  "outcomes": [ /* CallOutcome objects, each with both run_id and provider_call_id */ ],
  "stats": {
    "completed": 3,
    "total": 3,
    "escalated": 1,
...
| `escalate_on_negative` | `bool` | `True` | Misnamed — gates the *retry* branch of triage, not an escalation |
...
Note `integrations/voice/engine.TERMINAL` is a *separate*, lowercase set (`completed`,
...
| F42 | AI disclosure, consent | ⚠️ Partial | Disclosure is *copy* in built-in goals and settings UI. **Not enforced in a compiler** — a goal can be saved without one |
...
1. **CLOSED — `CallOutcome.run_id` used to hold the *provider's* call id, not the run id.**
...
8. **`escalate_on_negative` is misnamed** — it gates a *retry*, not an escalation.
