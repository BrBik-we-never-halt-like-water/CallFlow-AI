...
│   │   │   ├── api/v1/routes/      runs, voice_agents, telephony, telephony_numbers,
│   │   │   │                       organisations, profile, api_keys, integrations,
│   │   │   │                       ai_providers, suppressions, safety, escalations,
│   │   │   │                       sharing, messages, invitations, internal
│   │   │   ├── core/               config, rate_limit, crypto, logging
│   │   │   ├── auth/               tokens, dependencies, permissions
│   │   │   ├── database/           models, session, privileged, repositories/
│   │   │   ├── domain/             entities, safety, triage, result_schemas, numbers,
│   │   │   │                       collection, spreadsheet, prompt_assembly,
│   │   │   │                       number_allocation, api_keys  - pure, no I/O
│   │   │   ├── services/           run_dialer, run_dispatch, number_provisioning,
│   │   │   │                       number_directory, voice_preview (async)
│   │   │   └── integrations/       livekit/, email/, telephony/ - voice/protocol.py
│   │   ├── alembic/                env.py + 41 revisions
│   │   ├── tests/                  31 files, 642 tests
│   │   └── pyproject.toml
│   ├── voice-runtime/              LiveKit Agents worker - a third pm2 process
│   │   ├── app/                    worker (lifecycle, tools) · pipeline (plugin registry)
│   │   │                           · collection (fields, pure) · reporter · config
...
| **Safety guards**                 | **Removed.** The per-run ceiling, rate limit, daily budget and allowlist - and the page that edited them - are all gone, at the product owner's direction, pending a replacement security layer (`ISSUES.md` #178, CLAUDE.md §4 #8). The suppression list survives as the one per-dial guard; usage credit (org-wide and per teammate) is the one per-run guard                                                                                                                                                                                                                                                                                                                                                                                                                                                               | none (`GET/PATCH /api/v1/safety` deleted)                                                                                        | ❌ `org_safety_settings` is no longer read by anything - see §7                                                                                    |
...
| **Sharing** (Phase 4)              | Operator: browse a name-only, org-wide directory of teammates' open escalations, request to help (with an optional message). Owner: approve (hands the escalation off) or reject, from the Organisation page's "Sharing" tab - live-synced across teammates. Campaign sharing went with campaigns (ADR-8); an agent is not offered in its place, because agents are already org-wide readable and there would be nothing to request. The `share_requests_resource_type_valid` check now allows **only** `'escalation'` (migration `202608180900`), so 'campaign' is refused by the database, not merely unused by the UI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `GET .../escalations/directory`, `GET/POST /api/v1/share-requests`, `POST .../{id}/approve\|reject` | ✅ org-scoped Postgres row (`share_requests`); Realtime-pushed to requester and owner                                                                                              |
...
| ~~`core/rate_limit.py`~~                        | **Deleted** along with the rest of the cost guards (`ISSUES.md` #178). Was in-process sliding windows keyed per organisation; nothing rate-limits a dial today                                                                                                                                                                                       | -                                                                                                             |
...
| `auth/permissions.py`                           | The one role→permission matrix, plus the role-hierarchy checks for _which_ role a caller may grant and _whose_ row they may act on (owner > admin > operator > viewer). **`CAMPAIGNS_READ`/`WRITE`/`DELETE` are removed** - `AGENTS_READ`/`WRITE`/`DELETE` (every role / operator+ / admin+) replaced them with campaigns (ADR-8)                                                                                                                                                                               | `Permission`, `ROLE_PERMISSIONS`, `role_has()`, `can_grant_role()`, `can_act_on_member()`                     |
...
| `domain/entities.py`                            | Pydantic domain types and terminal-status sets. **No `dry_run` field anywhere**                                                                                                                                                                                                                                                                      | `Contact`, `RunAgent`, `CollectField`, `CallOutcome`, `AttemptSummary`, `Sentiment`, `Disposition`, `DialFailure` |
...
| `domain/result_schemas.py`                      | The shared triage contract every call inherits - `sentiment`, `wants_human_callback`, `do_not_call`, `frustration_signals`. **`build_result_schema()` has no caller left** outside this module's own `TRAVEL_SCHEMA`: a run now carries the agent's `collect_fields` to the worker as the `collect_schema` metadata key, and the worker collects them through its own tools rather than a JSON Schema handed to the engine | `BASE_PROPERTIES`, `build_result_schema()`                                                                    |
...
| `domain/spreadsheet.py`                         | Turning an uploaded sheet into contact rows, as pure functions over cell values - no file handling and no HTTP. `name`/`phone`/`note` are recognised by header name; **every other headed column becomes that contact's own context**, which is what lets one sheet produce a different conversation per person. Invalid rows come back flagged, never dropped - silently discarding one means a contact is never called and nobody finds out why. `apps/web/lib/contacts.ts` applies the same rules to a pasted CSV and the two must agree | `ParsedRow`, `SheetTooLarge`, `parse_rows()`, `to_context_key()`, `cell_text()`, `normalise_phone()`, `validate()`, `to_contact_payload()`, `KNOWN_HEADERS`, `MAX_ROWS` (20,000) |
...
| `domain/providers.py`                           | **The one provider list**, and the only place a vendor is described. 57 specs, each with its `roles` (a set - one Deepgram key does speech in *and* out), its own `CredentialField` list, and `runtime_extra`: the voice-runtime extra that can drive it, or `None` for a vendor CallFlow stores a credential for and nothing reads. Pure data - the database holds no allow-list of its own, so adding a vendor is an app change | `PROVIDERS`, `ProviderSpec`, `CredentialField`, `ProviderRole`, `for_role()`, `spec()` |
| `api/v1/routes/integrations.py`                 | `/api/v1/integrations/*` - the catalogue, plus connect (upsert)/list/disconnect and OpenRouter's OAuth exchange. Owner/admin only. `_validated_fields()` accepts exactly the keys a provider declared and **refuses** an unknown one rather than dropping it: silently discarding a field would let someone believe they had configured something they had not, and the only sign would be an authentication failure on a live call | `router`, `ProviderSpecOut`, `CredentialFieldOut` |
...
| `integrations/telephony/telnyx.py`              | **The only Telnyx REST caller.** One *credential connection* carries both directions where Twilio needs a trunk plus origination URLs, so this creates one object rather than three. The only adapter authenticating with a bearer token rather than HTTP Basic, and the only one that looks a number up by E.164 because Telnyx updates numbers by id | `TelnyxCarrier` |
...
**The dispatch metadata** is the contract between `RunDialer._originate()` and the worker's
`AgentSpec.from_metadata()`. Both halves live in this repo and are tested against each other
(`test_dispatch_contract.py`) because they were once written apart and every dispatched job
died resolving providers. The keys: `prompt`, `voice_agent_id`, `agent_name`,
`contact_name`, `phone_masked`, `result_schema`, `collect_schema`, `language`, `voice_agent`
and `max_call_duration_seconds`. Two of them carry the weight:

- **`voice_agent`** - the opaque provider half (`stt_provider`/`tts_provider`/`llm_provider`,
  `llm_model`, `voice_id` and the decrypted per-leg API keys), resolved once per run by
  `services/run_dispatch.py`. It holds key material, so nothing but the dialer and the worker
  looks inside it and it never reaches a log line.
- **`collect_schema`** - the agent's `collect_fields`, serialised, so the worker can build a
  `record_field` tool from the same list the prompt renders. Sent rather than re-derived
  precisely so what the agent is *asked* for and what it *can record* cannot drift.

`contact.context` is **stripped of reserved keys and spread first** into this dict, so an
uploaded spreadsheet column can neither overwrite the prompt nor misaddress the completion
callback - `collect_schema` is itself on the reserved list. Deliberately **no phone number**:
participant metadata is visible to everything in the room and reaches LiveKit's own logs and
webhooks, outside CallFlow's redaction. `contact_name` + `phone_masked` are included because
`call_outcomes` is keyed on `(run_id, contact_name, phone_masked)` - without them the
completion would insert a second row beside the in-flight one instead of resolving it.

**The worker side** (`apps/voice-runtime/app/`). `collection.py` is the pure half, kept apart
from `worker.py` for the same reason `pipeline.py` is - the LiveKit tool wiring needs a live
session to exercise and none of these decisions do:

| Piece                | What it does                                                                                                    |
| -------------------- | --------------------------------------------------------------------------------------------------------------- |
| `fields_from_metadata()` | Reads `collect_schema` off job metadata. Never raises - a malformed schema yields an empty list rather than killing a live call |
| `coerce()`           | A spoken answer into the declared type. Lenient where it should be ("about three thousand" survives a number field), strict about emptiness |
| `Collector`          | The values gathered during one call - mutable, one per `run_call`, holds no session and does no I/O. `record()` **refuses an undeclared key**, because an agent recording a field nobody declared has invented one, and letting that through puts a hallucinated field on a customer's record. `missing()` is the required fields still unanswered; `request_hangup()` sets a flag, first reason winning |
| `recover_missing()`  | After the call, scans the transcript for anything still absent. **Never overwrites a recorded value** - a recorded value is what the agent heard, a recovered one is a guess, and a guess that looks like an answer means the person who should have been asked never is. Bounded by `_MAX_SCAN_CHARS` (40,000) so a runaway regex cannot hold a job open past the room closing |

`worker.py`'s `build_tools()` gives the agent exactly two things it can *do*, built per call
because both close over that call's `Collector`: `record_field` - whose docstring has the
real field names substituted in, since the docstring *is* the tool schema the model sees, and
which is omitted entirely for an agent with no fields rather than offered as something it can
only misuse - and `end_call`, which exists because "please stop calling me" has to end the
call on the spot rather than after the agent finishes its sentence. That is wired through
`wait_for_call_end`'s `should_end` poll (`lambda: collector.hangup_reason is not None`):
without it the agent would set a flag and then wait for the contact to hang up on a call they
had already asked to end. The completion report happens in a `finally`, so a crash mid-call
still tells the operator what was said up to that point rather than leaving the row in flight
forever.
...
Base URL from `NEXT_PUBLIC_API_URL`. **Authenticated by default.** Five endpoints are
reachable without one, and the list is short on purpose - `GET /`, `GET /api/health`,
`GET /api/v1/invitations/{token}`, `GET /api/v1/public/billing/plans` (the plan ladder and
the gateway's live prices, for the marketing site, which has no session - it opens no
database connection and marks no plan `current`, so there is no tenant to resolve), and
`POST /api/v1/webhooks/dodo` (the payment gateway's callback, where a signature over the
raw body is the boundary instead of a token). Everything else requires a bearer token
...
`Permission.INTEGRATIONS_WRITE`. Body: `provider` (`twilio` | `plivo`), a
client-chosen `idempotency_key`, and **either** `number_id` **or**
`phone_number` + `label`. `number_id` is what the product sends: every number
the API returns is masked (non-negotiable #4), so no screen holds the E.164 to
send, and the route resolves the id against `telephony_numbers` where the
unmasked value is already permitted - 404 if it is not this org's, 400 if it is
on a different carrier than `provider` names. `phone_number` stays for API
callers with a number the org has never synced. `number_ref` (Twilio's Phone
Number SID, or the number itself for Plivo) defaults from the resolved row.
Returns **202** with the attempt row - `id`, `voice_agent_id`, `status`,
`last_error`, `created_at`, `updated_at` - and never a trunk id, since those are
LiveKit's internal handles and an operator can do nothing with them.

**A verified attempt writes back to the number itself.** `telephony_provisioning`
is the ledger of one attempt; `telephony_numbers` is the number, and
`GET /telephony/numbers` resolves `diallable` from *that* row. The service mirrors
each state change onto it - `provisioning` when the attempt starts, `verified`
plus every trunk id when it completes - which is what makes a synced number
dialable at all. Until this existed nothing ever wrote `verified` to a number row
and no run could dial from any line (`ISSUES.md` #149). A `disabled` number is
left out of service: `disabled -> verified` is a legal transition, so the state
machine alone would not stop provisioning from re-enabling a line an operator
...
`calling_available` is computed from the LiveKit configuration - it reports whether this
deployment actually has an origination path, rather than the hard-coded `false` it
carried while CALL-E was being removed. It replaces the old `api_key_configured` field,
which named a vendor key that no longer exists.
_Used by:_ `/status`.

### `GET`/`PATCH /api/v1/safety` - **removed**

Both endpoints, `routes/safety.py`, `repositories/safety_settings.py`, the
`org_safety_settings` read path, and Settings → Safety (`/app/settings/safety`) are all
gone. They configured the allowlist, the per-run ceiling, the rate-limit window and the
daily budget - every one of which was removed from `check_dial_allowed()` at the product
owner's direction, pending a replacement security layer (`ISSUES.md` #178, CLAUDE.md §4
non-negotiable #8). An endpoint that saves settings nothing reads is worse than no
endpoint: it tells an owner they have configured a guard they have not.

**What still gates a dial:** the suppression list, checked per dial inside
`check_dial_allowed()`. **What still gates a run:** the organisation-wide usage-credit
balance and the acting teammate's usage-credit cap, both checked in `POST /api/v1/runs`
before any dialling starts. Nothing else.
...
hashes is handed to `RunDialer` as `suppressed_hashes`, and `check_dial_allowed`
...
task (`_run_and_persist`) drives the async `RunDialer`, and on every progress callback
...
### `POST /api/v1/runs/parse-sheet`

Requires `Permission.RUNS_START`. Multipart (`file`), and the response is reviewed in the
composer before a run is started - **nothing is dialled and nothing is stored.** An upload
is never a dial.

Parsing is server-side rather than in the browser because a workbook is a zip of XML with
shared strings, styles and typed cells: a phone number typed as digits arrives as a float,
and getting that wrong turns a valid number into `9.19876543210e+11`. CSV is still parsed
in the browser (`lib/contacts.ts`), where it is a string split and the round trip buys
nothing - the two implementations apply the same rules on purpose, since a row the browser
accepts and the server rejects is a row someone has no way to fix.

Refusals, all before any parsing: an empty file (400), one over `MAX_UPLOAD_BYTES` = 5MB
(413), and anything that is neither `.xlsx`/`.xlsm` by extension nor a recognised
spreadsheet content type (400 - "Upload an .xlsx file, or paste the rows in as CSV."). A
sheet past `spreadsheet.MAX_ROWS` (20,000) is 413. The rules themselves are
`domain/spreadsheet.py`; this route only handles the file and the HTTP status.
  ...
  "rows": [
    {
      "row": 2,
      "name": "Asha Menon",
      "phone_masked": "+1 555 ***0142",
      "note": "asked about the Kerala package",
      "context": { "enquiry": "Kerala package", "budget": "80000" },
      "valid": true,
      "error": null,
      "contact": { "name": "Asha Menon", "phone": "+15550100142", "context": { "enquiry": "Kerala package", "budget": "80000" } }
    }
  ],
  "context_columns": ["budget", "enquiry"]
}
```

`phone_masked` is what the composer displays; the full number travels only inside
`contact`, which is the body `POST /api/v1/runs` accepts for that row and which the browser
posts straight back. An invalid row comes back with `valid: false`, its reason in `error`
and `contact: null` - flagged rather than dropped. `context_columns` is every non-`name`/
`phone`/`note` header found across the sheet, so the composer can say what each contact
brings into its own conversation. Only the row count and the column *names* are logged,
never a cell value.
...
[
  {
    ...
    "calls_closed": 20,
    "open_escalations": 2,
    "credit_cap_paise": 5000,
    "credit_spent_paise": 700
  }
]
```

**Superseded, kept as a marker: the per-teammate *call-count* allocation
(`daily_allocation`/`credits_used_today`) is retired** (`ISSUES.md` #146) - calls-per-day
stopped being a meaningful lever once usage credit became the thing plans are sold on, and
a second, money-blind allocation next to the real one was confusing rather than useful.
Only the money-denominated cap below remains per teammate; the org-wide daily budget
(§7) is unchanged, a uniform runaway safety rail rather than a packaging number.
...
this to the caller's own row. `{"credit_cap_paise": 5000, "credit_spent_paise": 700}` -
`credit_cap_paise` is `null` when nobody has set a share for this teammate, meaning only
the organisation-wide balance governs them; `credit_spent_paise` counts this calendar
month's holds and spends. Backs Settings → Billing's "My credits" view for anyone without
`billing:read`.

### `PATCH /api/v1/organisations/me/members/{user_id}/credit-cap`

Requires `Permission.CREDITS_WRITE` (owner, admin). Body
`{"monthly_credit_cap_paise": 5000}` (`>= 0`, or `null` to clear it). → `204`, `404` if
that user isn't a member, or `400` if the cap exceeds the plan's own per-period grant - a
share larger than the whole organisation ever receives is not a real ceiling on anyone.
**Enforced at the run gate**, alongside the organisation-wide balance
(`credit_service.member_credit_cap_status`, `ISSUES.md` #144): `POST /api/v1/runs`
resolves both once per run and refuses if either is exhausted. A teammate with no row at
all is not gated by this check - only an admin/owner explicitly setting a cap (including
`0`) turns it on for them.
...
```json
{
  ...
  "tts": [ { "id": "sarvam", "...": "...", "voice_options": ["ritu", "pooja", "simran", "kavya", "ishita", "shreya", "priya", "neha", "shubh", "rahul", "amit", "rohan", "dev", "aditya"], "voice_model": "bulbul:v3" }, { "id": "elevenlabs", "...": "..." } ],

**`voice_options` is a promise, and `voice_model` is what keeps it one.** A vendor
that ties its voices to a model version can replace the whole set between releases -
Sarvam did, between bulbul v2 and v3 - and an offered voice the runtime cannot build
is silently swapped for that model's default (`ISSUES.md` #166). `voice_model` names
the version the list belongs to, and `test_dispatch_contract.py` checks every entry
against the runtime plugin's own compatibility table for it. `apps/api` cannot import
`livekit.plugins` - that would invert the layering - so the test is the seam.
...
| `region`   | `str \| None`    | `None`   | Carried per contact. Nothing reads it on the dial path any more - it was a CALL-E setting, and there is no deployment-wide fallback left |
...
**`CollectField`** - one thing the agent has to establish while the call is happening. The
stored shape of a `voice_agents.collect_fields` entry, and the shape the worker reads back
off the `collect_schema` metadata key.

| Field         | Type   | Default    | Notes                                                                                    |
| ------------- | ------ | ---------- | ---------------------------------------------------------------------------------------- |
| `key`         | `str`  | required   |                                                                                          |
| `type`        | `str`  | `"string"` | One of `FIELD_TYPES` - `string`, `boolean`, `integer`, `number`                           |
| `description` | `str`  | `""`       | Load-bearing, not advisory: it becomes what a person is told to ask when the field is missing (ADR-5) |
| `required`    | `bool` | `False`    | A missing required field escalates an otherwise-clean completed call to a person (ADR-5) |

**`RunAgent`** - a voice agent as the run pipeline needs it, and the successor to
`Campaign`. A campaign owned the goal template, the result schema and the escalation
policy; an agent owns all three as `system_prompt` + `collect_fields`, configured once and
reused across runs (ADR-8). Deliberately narrower than the `voice_agents` row - the
provider choices travel separately as the opaque `voice_agent` metadata dict, so the domain
never holds a decrypted credential. Named `RunAgent` and not `AgentSpec` because the voice
runtime already has an `AgentSpec` for the provider half.

| Field                  | Type                 | Default  | Notes                                                                                        |
| ---------------------- | -------------------- | -------- | -------------------------------------------------------------------------------------------- |
| `id`                   | `str`                | required |                                                                                              |
| `name`                 | `str`                | required |                                                                                              |
| `system_prompt`        | `str \| None`        | `None`   | What the agent is told before this contact's own detail is added. Supports `{name}` and `{context_key}` placeholders - `domain/prompt_assembly.py` |
| `collect_fields`       | `list[CollectField]` | `[]`     | What the agent has to come back with. An empty list is a conversation-only agent, which is legal |
| `language`             | `str \| None`        | `None`   |                                                                                              |
| `escalate_on_negative` | `bool`               | `True`   | Whether a negative-sentiment call is worth one retry or a person. Carried on the agent now that campaigns are gone; `triage()` still reads it as a parameter rather than reaching for the agent itself |
| `required_field_keys`  | property             | -        | The `collect_fields` whose absence sends a call to a person (ADR-5)                           |
...
| `phone_masked`       | `str`            | required    | **Already masked** - the raw number never leaves `RunDialer`                                                                                                                                                                                                                                                                                                     |
...
| `run_id`             | `str \| None`    | `None`      | Set by `run_dialer.py` to the **provider's own call id** while a call is in flight - this domain object never held the run id. `routes/runs.py` fixes this at the persistence boundary: it remaps this field to `provider_call_id` before writing, and the API always serves the real run id in its place (see §13, ISSUES.md #1, now closed at the API surface) |
...
**All of these are persisted and served.** `collected`, `missing_required_fields`,
`handoff_questions` and `from_number_masked` were declared here, typed in `api.ts`
and rendered by `TranscriptView` long before `append_outcome` wrote them or
`get_run` returned them - so every run showed an empty "What we asked for" while
the escalation queue read `{}` for the fields that raised the escalation
(`ISSUES.md` #155). `from_number_masked` is coalesced on update: the dialler knows
which line carried the call and the worker does not, so the terminal write must
not blank it.
...
`build_result_schema()` still composes an agent's own fields on top of these, but **nothing
calls it any more** - its one remaining use is this module's own `TRAVEL_SCHEMA`, and its
docstrings now describe agents rather than campaigns. Since ADR-8 an agent's
`collect_fields` reach the
worker as the `collect_schema` metadata key and are collected by the worker's `record_field`
tool, so no JSON Schema is handed to a provider for extraction at all.

**These six are consequently not populated today.** `completion_payload()`
(`apps/voice-runtime/app/worker.py`) sends `"extracted": {}` deliberately - nothing yet
infers sentiment or the escalation signals from the conversation, and sending invented
values would decide a disposition on them. `triage()` therefore falls through to the
status buckets, and the agent's own fields arrive in `collected` instead. Wiring an
inference step is the outstanding work; the contract above is what it has to fill.
...
`Contact`/`RunAgent`/`CallOutcome` above, they're plain asyncpg rows shaped only by
...
| Guard                       | Default                                                    | Where                                                                                         |
| --------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Suppression list            | on, always checked                                         | `is_suppressed` param, resolved once per run against `public.suppressions`. **The only per-dial guard left** |
| Organisation-wide usage credit | the plan's per-period grant                             | `check_credit_available()`, at the **run gate** (`POST /api/v1/runs`), before any dialling starts             |
| Per-teammate usage-credit cap | unset (no per-teammate ceiling; the organisation-wide balance is the only gate) | `member_credit_allocations.monthly_credit_cap_paise`, editable admin/owner via `PATCH .../members/{id}/credit-cap`. Money in paise, checked against this period's spend at the run gate alongside the org-wide balance (`ISSUES.md` #144) |
| Phone masking               | always                                                     | `safety.mask` + `lib/format/phone.ts`                                                         |

**Removed, deliberately** (`ISSUES.md` #178, CLAUDE.md §4 non-negotiable #8): the
allowlist, the per-run ceiling, the per-organisation rate limiter, the daily budget,
E.164 validation *as a dial gate*, the per-teammate daily call allowance
(`daily_allocation`, `ISSUES.md` #146), and the `X-CallFlow-Owner-Key` bypass that
existed to lift them. `core/rate_limit.py`, `repositories/safety_settings.py`,
`routes/safety.py` and `resolve_safety_settings()` are all deleted, along with the
`GET/PATCH /api/v1/safety` surface and Settings → Safety. A replacement security layer
is being designed; until it lands, the suppression list is the only thing checked per
dial, and the two credit checks are the only thing checked per run.

`is_e164`/`assert_e164` still exist in `domain/safety.py` and are still used - but as
*input validation* on uploads, not as a gate that refuses a dial.
...
### Routes (34 `page.tsx` + 8 docs MDX → 48 built route entries, 51 counting the 4 prerendered `/solutions/[vertical]` variants; verified via `next build`)
...
- **`(app)/app`** - dashboard (`/app`), `agentic` + `new` + `[id]` (the Agents tab - the voice-agent builder), `runs` + `new` + `[id]`, `escalations`, `contacts`, `chat` (internal team chat - channels/DMs, §3/§5; `chat-shell.tsx` is the component the route renders, parameterised by the `?c={id}` query param, not a `[id]` segment - see the Team chat row in §3 for why), `integrations`, `billing`, `profile`, `organisation` + `new`, `settings` + panes (`api-keys`, `billing`, `integrations`, `team`), `platform` (cross-tenant support, staff-only - `docs/PLATFORM_ADMIN.md`; `is_platform_admin` from `GET /me` is the affordance only, every route behind it 404s without a `platform_admins` row and the definer functions check again in the database). **`campaigns` + `new` + `[id]` and `settings/safety` are deleted** - campaigns went with ADR-8 (an agent is what a run binds to now) and the dial-gate settings page went with the safety-gate simplification (§4b/CLAUDE.md non-negotiable #8). `/app/settings/integrations` and `/app/settings/billing` survive only as redirects to the top-level `/app/integrations` and `/app/billing`, both of which moved into the primary nav - connecting a carrier/speech vendor and seeing the plan that gates the rest of the product are both things a new organisation needs before anything else works, not settings to bury two clicks deep. The left column is `DashSidebar` (`components/app/dashboard/dash-sidebar.tsx`), grouped rather than one flat list - Workspace (Dashboard/Agents/Runs/Needs a person), Communication (Messages), Platform (Integrations/Data/Billing/Settings, permission-filtered per item), and - appended only for `profile.is_platform_admin`, absent from the static list entirely rather than filtered out of it - a Staff group holding the one `/app/platform` link. Organisation and Team are their own route, `/app/organisation` (`Suspense`-wrapped for `useSearchParams()`, `?tab=team`/`?tab=sharing` select the Team/Sharing panes) - not a dialog and not a Settings pane; `/app/settings` and `/app/settings/team` both redirect to `/app/settings/api-keys` so pre-existing generic "Settings" links still resolve; "Organisation" and "Settings" live in the account menu (`user-menu.tsx`'s dropdown) and in the sidebar's account row, and are the only two `NAV_ITEMS` entries `PRIMARY_NAV_ITEMS` filters out - they are lower-frequency than the rest, while Chat, Agents, Integrations and Billing are working features someone checks often. Creating a second organisation is a dedicated two-step page, `/app/organisation/new` (name, then an optional logo - logo upload has to be a second step because Storage RLS scopes the upload path by `org_id`, which doesn't exist until the create call returns). There is no `/app/welcome` - the mandatory org-setup step + its skippable profile follow-up are **not routes at all**; `OnboardingGate` renders them as a modal over whatever page is active (see §3/§12), specifically to avoid the two-independent-`useSession()`-instances bug a page-per-step version had (`ISSUES.md`)
...
### `lib/` (14 top-level files, plus `format/`, `hooks/`, `auth/`, `supabase/`, `sound/`)
...
| `brand-assets.ts`                          | The `--dark-*` palette as literal hex, for the favicon, both `next/og` images and the manifest - the four assets that cannot read CSS. Checked against `globals.css` by `scripts/check-tokens.mjs`; all four had gone three versions stale (`ISSUES.md` #187) |
...
| `collect-fields.ts`                        | The agent editor's field model - 5 editor types → 4 wire types (`date`/`enum` fold onto `string`, their constraint moved into the description, which *is* the extraction instruction). The successor to `campaign-fields.ts` |
| `run-settings.ts`                          | Calling window, timezone and retry policy in `localStorage`, keyed per organisation. The successor to `campaign-draft.ts`: these moved off the campaign onto the run (ADR-8), and are held locally because **no API field accepts them yet** - the composer's guard bar reads them so the safety story isn't shown narrower than it is (`ISSUES.md` #20) |
| `agent-draft.ts`                           | An in-progress agent in `localStorage`, so a half-built agent survives a navigation away from the editor                                                                              |
...
| `hooks/use-connection.ts`                  | Loads health + **voice agents** (`api.listVoiceAgents()`) via `useOrgScopedEffect`, so switching organisation re-runs the whole connect sequence rather than showing the previous org's agents; 2 quick retries at 1.5s absorb a blip. No cold-start wake logic - the VM deploy is always on, so a failure here means something is actually wrong |
...
| `dialog.tsx`                             | `Dialog` and `Sheet`, shared overlay, focus trap. Four sizes - `sm`/`md`/`lg` plus **`full`**, the record-inspection size: a near-full-viewport panel (`h-[90dvh] w-[80vw]`) for content that has to be read alongside itself, a transcript next to the fields extracted from it. Used by `/app/contacts`' record dialog. `dismissible={false}` is the mandatory-step mode - no close button, no Esc, no click-outside                                                                                                                 |
...
The home page runs **scale first, then depth**: the hero shows a whole run, and
`problem-compare` -> `live-extraction` shows one call of it being understood.
That split is deliberate and load-bearing - it is what stops the two strongest
sections repeating each other (`DESIGN_NOTES.md` §28, `ISSUES.md` #186).
`listening.tsx` was deleted for having no axis of its own, and `voice-wave.tsx`,
`step-flow.tsx` and `brand/speech-wave.tsx` went with it as dead code.

| File                     | Role                                                                                                                                                                                                                                                    |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hero.tsx`               | The claim, plus `CallBoard`. Makes **no API call at all**; never shows an error, because the scripted run is all there ever was                                                                                                                            |
| `call-board.tsx`         | A run in progress - nine rows, lamps settling, a counter climbing. Every row is a pure function of one second counter, so server and client agree on first paint and `prefers-reduced-motion` is the same function frozen. Guarded by `scripts/check-board.mjs` |
| `problem-compare.tsx`    | `SectionHeading` over `LiveExtraction` - the "why not just read the log" objection                                                                                                                                                                        |
| `live-extraction.tsx`    | The same call resolving two ways side by side, on one clock: a log that says `completed` against typed fields with the spoken phrase each was read from. Four scenarios, five phases, complete and static under `prefers-reduced-motion`                    |
| `live-lamp.tsx`          | One lamp cycling dispositions, for `steps.tsx`                                                                                                                                                                                                           |
| `section-deck.tsx`       | `SectionDeck` / `DeckSection` - the scroll-snap deck and its depth transition. One ground throughout (§27); no `ground` prop                                                                                                                              |
...
| `pricing-preview.tsx`    | 3-plan preview                                                                                                                                                                                                                                           |
| `roi-calculator.tsx`     | Leads with **hours**, not money. Solution pages only                                                                                                                                                                                                     |
| `final-cta.tsx`          | Closing card, `LampStrip` with counts, waves fading down from the top edge                                                                                                                                                                                |
...
| `share-request-dialog.tsx` | `ShareRequestDialog` - Phase 4. "Request to help" against an escalation, optional 280-char message, `POST /api/v1/share-requests`. No longer varies by resource: campaign sharing went with campaigns (ADR-8), so escalations are the only `resourceType` left. The server still resolves who owns the target (`resolve_resource_owner()`) - this dialog never sends that itself |
...
**`app/agentic/` (8)** - the Agentic tab's builder-specific components, shared by the
create/edit routes (`/app/agentic/new`, `/app/agentic/[id]`).

| File                          | Role                                                                                                                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent-editor.tsx`            | `AgentEditor` - the shared create/edit form both routes wrap: name, the three provider legs (STT/TTS + voice, LLM model), the system prompt, and the fields to collect. **No number here** - which line a call comes from is chosen per run (ADR-8), which is what lets one agent dial through any carrier the org has connected. Seeded from `lib/agent-draft.ts`; a `blocker` string names the exact thing standing between here and a save |
| `provider-wheel.tsx`          | `ProviderWheel` - one leg of the agent (transcriber, model, or voice) as a scroll wheel over the provider catalog, with the centred option's details beneath it. Replaced the card-grid `ProviderPicker` |
| `agent-metrics.tsx`           | `AgentMetrics` - one live readout of what the current three-leg combination costs and how long a round trip takes, recomputed as the wheels move. The stacked bar is colour-coded by the `--leg-*` spectrum (CLAUDE.md §4 #10), never by a lamp colour |
| `agent-presets.tsx`           | One-tap starting points for the three wheels - each a *rule* over the catalog (cheapest/fastest/best), not a hard-coded provider list, so a preset cannot name a provider the catalog no longer carries |
| `collect-fields-editor.tsx`   | `CollectFieldsEditor` - add/remove/reorder the fields the agent has to come back with, over `lib/collect-fields.ts`'s 5 editor types |
| `provider-icons.tsx`          | `ProviderIcon` - per-vendor marks (Simple Icons), so a wheel entry is recognisable before its name is read |
...
| `/app/runs/new`                                    | Four stacked steps on one page, not a wizard: pick the **agent**, pick the **number(s)** it calls from (one, some, or all - calls are spread across whatever is chosen), load **contacts** (paste CSV/TSV, or drop an `.xlsx`/`.xlsm`, which round-trips through `POST /api/v1/runs/parse-sheet`), then start. Remove-invalid and per-row inline errors as before. **There is no goal preview** - the instruction lives on the agent now (ADR-8), so there is nothing to render locally. `?agent=` pre-selects what a "Run" button sent it. **No dry-run toggle; Start run always dials**                                      | `POST /api/v1/runs`                                                                             |
...
| `/app/platform`                                    | CallFlow staff only: every organisation with its plan and usage, an entitlement-override editor whose every save requires a reason, and the audit log. The nav entry renders on `is_platform_admin` from `GET /me`, which is display only - each route 404s without a `platform_admins` row and the definer functions re-check in the database | `GET /api/v1/platform/organisations`, `GET .../organisations/{id}/entitlements`, `PUT .../plan`, `PUT .../entitlements`, `GET .../organisations/{id}/runs`, `GET /api/v1/platform/audit` |
| `/app/billing`                                     | The plan, entitlement meters, plan cards with live gateway prices, checkout/switch/cancel, and payment history. A sidebar destination, not a Settings pane, since every 402 points here; `/app/settings/billing` redirects, because that path is baked into checkout sessions already minted at the gateway. Without `billing:read` the page is the personal "My credits" view instead | `GET /api/v1/billing/plans`, `GET /api/v1/billing/subscription`, `POST /api/v1/billing/{checkout,change-plan,cancel,sync}` |
...
| Organisation setup gate, team, API keys, Integrations (credential storage + carrier numbers), Billing (plan + usage), Suppression list | **Real** - see §3. Integrations now carries a number from synced to diallable (`ISSUES.md` #149/#150); placing a call over it additionally needs the deployment's LiveKit credentials. Billing has no payment processor |
...
| `CALLFLOW_MAX_CONCURRENT_CALLS`              | `5`     | How many contacts in one run may be dialling at once (`RunDialer.run()`'s `asyncio.Semaphore`). Starts conservative                                                                                     |
...
**Tests** - **642 collected across 31 files** in `apps/api`, plus **68 across 4 files** in
`apps/voice-runtime` (both verified via `pytest --collect-only -q`). `pytest -q`,
`ruff check app tests`. Every count in the table below was re-collected for this pass;
`voice-runtime/` rows are the separate suite, run from `apps/voice-runtime`.
...
| `test_provider_catalogue.py`   | 24    | The catalogue and the field validation the connect form depends on, with no database. Every provider has at least one field and at least one **secret** field (which is how Zapier's Catch Hook URL was caught rendering unmasked - it is a capability, not an address); field keys are unique per provider; only OpenRouter claims a login flow; a stored-only vendor never appears in a call role; an unknown field is refused rather than dropped; and credentials round-trip through one ciphertext that leaks neither values nor field *names*, with the pre-migration two-column shape still readable |
| `test_provisioning_state.py`   | 28    | The provisioning state machine, exhaustive over the whole 4x4 status grid - every declared move allowed, every other rejected, terminal states final, and the error message naming the legal alternatives. Pure, so this runs in CI |
...
| `test_permissions.py`          | 44    | `can_grant_role()`/`can_act_on_member()` - pure, no database - `ISSUES.md` #43. Plus `role_has()` pins for `Permission.RUNS_READ_TEAM` (owner/admin/viewer yes, operator no)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `test_triage.py`               | 19    | Precedence rules - the most thoroughly tested module                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `test_spreadsheet.py`          | 20    | The sheet-parsing rules, pure. The cases worth pinning are the ones where a spreadsheet lies about its own contents: Excel stores a phone number typed as digits as a float, so a valid number arrives looking like scientific notation; a column named `goal` would land in the same metadata dict the worker reads its instructions from; and an unrecognisable header row silently drops the per-contact context that makes each conversation different |
| `test_parse_sheet_route.py`    | 12    | `POST /api/v1/runs/parse-sheet` - what it accepts and what it refuses. Calls the route function directly (as `test_internal_completion.py` does) since it touches no database. Fixtures are real workbooks built in memory with `openpyxl`: the whole reason this parsing is server-side is the float problem above, and only a genuine round trip through the format demonstrates it |
| `test_collection.py`           | 19    | Whether a call came back with what the agent was asked to find out - the API half (`domain/collection.py`), pure. ADR-5's gap was that `required` was set on a field and then read exactly once to build a schema; nothing ever re-checked the answer against it, so a call could return none of the required fields and still record as `auto_closed` |
| `test_prompt_assembly.py`      | 17    | What one contact's agent is told. The load-bearing case is the one the product exists for: one agent, three contacts, three different conversations |
| `test_number_allocation.py`    | 9     | Which line places each call. Pure - the distribution across an org's numbers is the whole reason to hold several, so that is what is asserted rather than "a line came back" |
| `test_number_state.py`         | 33    | The `telephony_numbers` status machine, pure. What is pinned is not that the declared moves work but that **everything else is refused** - the illegal edges no feature test would ever exercise |
| `test_telephony_number_listing.py` | 18 | Listing the numbers an org already owns, per carrier, against stubbed HTTP. The pagination is the point: every vendor pages differently, and a loop reading only the first response returns a truncated picker with nothing to say it was truncated |
| `test_logging.py`              | 16    | The global redaction filter and correlation-context plumbing - the test CLAUDE.md's non-negotiable #5 points at |
| `test_pool_timeout.py`         | 6     | An exhausted pool must fail fast, not hang: a timeout is actually handed to the pool, and the resulting failure surfaces as a distinct type the API turns into a 503 (`ISSUES.md` #122) |
| `test_voice_preview.py`        | 6     | Preview orchestration resolves every failure path to an honest `PreviewResult` - never raises, never fabricates success. Conflating "you haven't connected a key" with "we haven't built this preview yet" is exactly the non-negotiable #9 case |
| `test_email.py`                | 5     | `EmailGateway`'s Resend integration: request shape and error translation. Domain verification is the rejection worth naming, since no retry works around it |
| `voice-runtime/test_collection.py` | 19 | Field collection in the worker: what the agent records, and what is read back afterwards. The interesting cases are where leniency and safety disagree - "about three thousand" is a real answer to a number field and must survive, a field the agent invented must not, and recovery must never displace a value the agent actually heard |
...
| F2      | Database schema                                         | ⚠️ Partial | Core relational schema is real: `users`, `organisations`, `memberships`, `suppressions`, `runs`, `call_outcomes`, `api_keys`, `provider_credentials`, `ai_provider_credentials`, `org_safety_settings`, `voice_agents`, `telephony_numbers`, `telephony_provisioning`, `channels`, `messages`, `escalations`, `share_requests`, all RLS'd. **`campaigns` is dropped** (migration `202608180900_agent_driven_runs_replace_campaigns`), which also narrowed `share_requests_resource_type_valid` to `('escalation')`. No credit ledger, billing, or audit-log tables yet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| F15     | ~~Campaigns~~ → Voice agents                             | ⚠️ Partial | **Superseded, not delivered.** Campaigns are gone entirely (ADR-8, migration `202608180900`): the table is dropped, `domain/campaigns.py` and `services/campaign_runner.py` are deleted, and the editor/card/routes with them. A voice agent is what a run binds to now - real org-scoped `voice_agents` rows, CRUD + provider catalogue + `collect_fields`, `agents:read/write/delete` permissions. Still no versioning, and no prebuilt agents (the "Prebuilt" tab is honestly unwired) |
...
| F20     | Call execution                                          | ⚠️ Partial | **The dial path is connected.** `RunDialer.run_one()` clears the safety gate, allocates a line, renders the per-contact prompt and originates over SIP through `LiveKitGateway.start_call()` (`CreateSIPParticipantRequest`), then returns `IN_PROGRESS`/`IN_FLIGHT` - it no longer returns `provider_unavailable` for every contact. The conversation happens in the agent worker (`apps/voice-runtime`), which POSTs the transcript and terminal status back to `/internal/v1/runs/{run_id}/complete`; `run()` dials up to `CALLFLOW_MAX_CONCURRENT_CALLS` at once. `services/run_dispatch.py` resolves the agent, the lines and the decrypted provider keys, and **fails closed** with a sentence naming what to fix. What is still missing: nothing acts on the `RETRY` disposition, no billable flag or provider cost is recorded, and **`extracted` arrives empty** - the worker infers no sentiment or escalation signals yet, so triage falls through to the status buckets (see §6). None of this has been exercised against a live carrier account |
| F21     | Typed extraction                                        | ⚠️ Partial | No transcript scraping, and the field list travels to the worker as `collect_schema`, which builds the `record_field` tool from it - so a value is recorded as the contact says it. `domain/collection.py` re-checks the answers against `required` (ADR-5) and `apps/voice-runtime/app/collection.py`'s `recover_missing()` reads anything the agent forgot to record back out of the transcript, never overwriting a recorded value. **But the provider-side `result_schema` extraction is gone**: `build_result_schema()` has no live caller, and the shared triage contract (`sentiment`, `wants_human_callback`, `do_not_call`, `frustration_signals`) is consequently unpopulated - the worker sends `"extracted": {}` on purpose rather than invent values. Results are not validated against a schema on return |
| F22     | Triage engine                                           | ⚠️ Partial | Pure, precedence-ordered, 19 tests. No custom org rules - **and its main inputs no longer arrive**: with `extracted` empty (F21), only the status-bucket and `missing_required_fields` branches actually fire today |
...
| F26     | Analytics                                               | ⚠️ Partial | Dashboard stats computed client-side; no rollups and no per-agent comparison                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
1. **CLOSED - `CallOutcome.run_id` used to hold the _provider's_ call id, not the run id.**
   ...
   One internal wart remains - `RunDialer`/`CallOutcome` still uses the `run_id`
   field as scratch space for the vendor's call id while a call is in flight
   (`services/run_dialer.py`, where the in-flight outcome is built with
   `run_id=created["sip_call_id"]`); only `routes/runs.py` remaps it before
   persisting.
...
9. **`build_result_schema()` has no caller.** The dead code is now one layer down: the
   `POST /api/v1/campaigns/preview` endpoint, its `api.preview` client method and the local
   `renderGoalPreview` are all gone with campaigns (ADR-8), and there is no goal preview to
   render - the instruction lives on the agent. What is left behind is
   `domain/result_schemas.py`: `build_result_schema()` is called only by its own
   `TRAVEL_SCHEMA`, and its docstrings still describe campaigns. The six-field triage
   contract it composes is real and `triage()` still reads it, but nothing populates it
   (§6, F21) - so this is a genuine gap, not merely unused code.
...
### `provider_credentials.verified_at` (`d1a83c5f27e6`)

When the vendor last confirmed *these* credentials. Written only where
`services/credential_check.py` returned `ok=True`, so:

- **not null** - the vendor accepted them at that moment. The only state the
  interface reports as "Connected", and the only one the readiness line counts
  towards "ready to place a call".
- **null** - stored but unconfirmed: `ok=False`, or `ok=None` (no probe, an
  unreachable vendor, or a key too narrowly scoped to check), or a row written
  before the column existed.

Stores *when* rather than *whether*, because a boolean cannot separate
"confirmed a minute ago" from "confirmed in March", and a key revoked at the
vendor's end is exactly where that difference decides whether "Connected" is
true. Nullable, no default, no backfill: `now()` as a default would assert that
every row already in the table had been checked - the same false claim the
column exists to remove - and a backfill cannot check anything without the
encryption key (`c8e1f4a29b76` on why a migration must not need one). No RLS or
grant work: the table already forces row level security and `f6a3c9d1b527`
grants at table level rather than per column.
