...
| Verified    | 30 API endpoints (all but 3 authenticated) · 50 built routes · **182 backend tests** (31 cross-tenant/cross-role RLS) · eslint + `tsc` clean · `alembic check` no drift   |
...
| **Runs**                          | Start (always live, idempotent via `Idempotency-Key`), watch live, cancel (stops the next contact, not one already in conversation), open a call's transcript, export CSV                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `POST /api/v1/runs`, `GET /api/v1/runs`, `GET /api/v1/runs/{id}`, `POST /api/v1/runs/{id}/cancel`                                                                  | ✅ org-scoped Postgres, updated as each call resolves; a process restart fails any run left `running`/`canceling`                                                                                                                               |
...
| `/app/runs/[id]`                                   | Watch lamps settle, pause updates (browser polling only), cancel the run for real (stops the next contact from being dialled, not one already in conversation), open transcript sheet                                                                                                              | `GET /api/v1/runs/{id}` every 2.5s, `POST /api/v1/runs/{id}/cancel`                                                              |
